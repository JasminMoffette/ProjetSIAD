...
class SaputoProbleme1(probleme.Probleme):
    ...
    def setLiens(self):
        ...
        for produit in self.fichier.produits:
            ligne = produit.ligne
            nombre_ligne = 0
            for element in fichier.dict_cadence.keys():
                if ligne == element:
                    self.liens.append((nombre_lot, nombre_ligne))
                    break
                nombre_ligne+=1
            nombre_lot+=1
...
fichier.setEmployes()
fichier.setDictCadence()
fichier.setDictEmployes()
fichier.setDictOperateurs()
fichier.setJournee()
